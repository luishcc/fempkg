...
class InOut:
 ...
 def saveVTK(_self,_dir,_file,_iter=None):
  ...
  if _self.scalar2 is not None:
   _self.vtkScalar(vtkFile,"StreamFunction",_self.scalar2);

  if _self.scalar3 is not None:
   _self.vtkScalar(vtkFile,"Vorticity",_self.scalar3);

  if _self.scalar4 is not None:
   _self.vtkScalar(vtkFile,"Temperature_analytic",_self.scalar4);

  if _self.scalar5 is not None:
   _self.vtkScalar(vtkFile,"Vx_analytic",_self.scalar5);
