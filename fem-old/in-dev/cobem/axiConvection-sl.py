...
savename = mesh_file + "-sl"
# savename = "axi_cond_2d" + "-sl"
...
for t in range(0, time):
    ...
    temp_dep = sl.Linear2D(nodes_global, neighbour, ien_global, x_global,
                           y_global, vz, vr, dt, temp_old)

    # --------------------------------------
    # RHS matrix with Dirichlet BC

    RHS = sp.dot(Mdt, temp_dep) + cct
    for i in range(dirichlet_len):
        index = int(global_mesh.dirichlet_points[i][0]-1)
