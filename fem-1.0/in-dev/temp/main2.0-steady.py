...
for i in range(nodes):
    T_a[i] = -15./48. + 24*malha.X[i] + 1.5*malha.Y[i]**2-malha.Y[i]**4


print "Generating .VTK file(s)"
vtk = io.InOut(x, y, ien, len(x), len(ien), Tfim, T_a, None)
vtk.saveVTK(cwd + "results", arquivo+"-steady")
