    ...
    def __init__(self, mshfilename):
        ...
        for line in self.mshfid:
            ...
            if reading_nodes == 2:
                ...
                reading_nodes = 3
            if reading_nodes == 3:
                sl = sp.array(line.split(), dtype=sp.float64)
            ...
            if reading_elements == 2:
                self.IEN = sp.zeros((self.Nelem,3), dtype=sp.uint)
                self.Boundary = sp.zeros((self.Nelem,2), dtype=sp.uint)
                reading_elements = 3
            if reading_elements == 3:
                sl = sp.array(line.split(), dtype=sp.int64)
        ...
        self.Boundary_Neumann = sp.zeros((boundarysize, 2), dtype=sp.uint)
